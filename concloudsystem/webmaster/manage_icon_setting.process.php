...
switch ($act) {
	...
	case 'edit_webmaster_background':
		...
		if (!empty($_FILES['videoOrGif'])) {
			...
			regFailureImageUpload($id, $_FILES['videoOrGif'], 'WEBMASTER_BACKGROUND', $mycms, $cfg);
				...
				echo '<script>window.location.href="company_info.php#br";</script>';
    ...
    $brandingInputs = [
        ...
        'qr_code'              => 'QR_CODE',
		'success_image'        => 'Registration Online Success Image',
		'process_image'        => 'Registration Processing Image',
		'registration_logo_image'        => 'Landing Page Outer Background Image'
    ];
    ...
    foreach ($brandingInputs as $inputName => $uploadType) {
        ...
        if (!empty($_FILES[$inputName]['name'])) {
            ...
            if (!empty($fetchData[0]['image'])) {
                ...
                $oldPath = _BASE_URL_. $cfg['EMAIL.HEADER.FOOTER.IMAGE'] . $fetchData[0]['image'];
